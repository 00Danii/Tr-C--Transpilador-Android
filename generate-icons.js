...
async function generateIcons() {
  ...
  try {
    ...
    await sharp(basePath).resize(1024, 1024).toFile(path.join(outputDir, 'icon.png'));
    console.log('Generado: icon.png');

    // Android foreground (432x432, asume transparente)
    await sharp(basePath).resize(432, 432).toFile(path.join(outputDir, 'android-icon-foreground.png'));
    console.log('Generado: android-icon-foreground.png');

    // Android monochrome (432x432, convertir a gris)
    await sharp(basePath).resize(432, 432).greyscale().toFile(path.join(outputDir, 'android-icon-monochrome.png'));
    console.log('Generado: android-icon-monochrome.png');

    // Favicon (32x32)
    await sharp(basePath).resize(32, 32).toFile(path.join(outputDir, 'favicon.png'));
    console.log('Generado: favicon.png');

    // Splash icon (200x200, si quieres cambiar)
    await sharp(basePath).resize(200, 200).toFile(path.join(outputDir, 'splash-icon.png'));
    ...
    }).png().toFile(path.join(outputDir, 'android-icon-background.png'));
